...
describe("MarketItem unit tests", () => {
  it("should throw error when id is empty", () => {
    expect(() => {
      const mkItem = new MarketItem("", "title", new Date(), 1);
    }).toThrowError("Id is required");
  });

  it("should throw error when title is empty", () => {
    expect(() => {
      const mkItem = new MarketItem("id1", "", new Date(), 1);
    }).toThrowError("Title is required");
  });

  it("should throw error when dateOfPost is empty", () => {
    expect(() => {
      const mkItem = new MarketItem("id1", "title", null as any, 1);
    }).toThrowError("DateOfPost is required");
  });

  it("should change description", () => {
    const mkItem = new MarketItem("id1", "title", new Date(), 1);
  ...
  it("should throw error when price is empty", () => {
    expect(() => {
      const mkItem = new MarketItem("id1", "title", new Date(), null as any);
